...
let statusDiv = document.getElementById('estado');
...
function subscribe(uri) {
  ...
  function connect(uri) {
    ...
    events.addEventListener("error", () => {
      ...
      console.log(`desconectao. bamo a intenta que te reconektes en ${timeout} segundos`);
...
function setConnectedStatus(status) {
  ...
  statusDiv.className = (status) ? "conectao" : "reconectando";
}
...
function init() {
  ...
  addRoom("zona sexo");
  changeRoom("lobby");
  addMessage("lobby", "irone😳", "ABRE OTRA PESTAÑA O MUERES aunque tambien puedes enviar un mensaje jeje.", true);
  addMessage("zona sexo", "irone😳", "la zona del sexo", true);
  ...
  newMessageForm.addEventListener("submit", (e) => {
    ...
    const username = usernameField.value || "invitado";
  ...
  newRoomForm.addEventListener("submit", (e) => {
    ...
    addMessage(room, "irone😳", `mira, tu propa sala de nombre "${room}" codeada por irene jeje que bonito`, true);
  })
